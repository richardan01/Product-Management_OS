...
│  /briefing  /today  /prd  /risk-register  /go-nogo      │
└──────────────────────────┬──────────────────────────────┘
                           │ invokes
┌──────────────────────────▼──────────────────────────────┐
│  TIER 2: Agents                                          │
│  Batman layer: Agents/Gotham/Computer/ (12 agents)       │
│  Day-job layer:   Agents/[name]/[name].md (optional)     │
...
|-----------|-------|-----------|---------|
| task-analyzer | Haiku | `/briefing`, `/eod`, `/sprint-review` | Parse tasks, calculate velocity |
| meeting-scanner | Haiku | `/briefing`, `/follow-ups`, `/meeting-prep` | Scan meetings, extract action items |
| metrics-reader | Haiku | `/briefing`, `/weekly-update`, `/dashboard` | Read metrics from files + MCPs |
| risk-reader | Haiku | `/briefing`, `/risk-register` | Scan risks, fire escalation triggers |
| research-worker | Sonnet | `/competitive-scan`, `/vendor-eval` | Web research (isolated context) |
| file-analyzer | Haiku | `/os-check`, `/data-quality` | Validate file structure and quality |
| project-scanner | Haiku | `/[anchor-project]-status`, `/roadmap-review` | Assess milestones and risks |
| stakeholder-profiler | Sonnet | `/meeting-prep`, `/check-in` | Build person context from history |
| prd-drafter | Sonnet | `/prd`, `/use-case`, `/business-case` | Draft structured documents |
| tech-reviewer | Sonnet | `/tech-feasibility`, `/tech-spec` | Analyze technical architecture |
| notes-reader | Sonnet | `/self-review`, `/os-check` | Read session JSONL and notes; extract facts, decisions, untracked commitments |
