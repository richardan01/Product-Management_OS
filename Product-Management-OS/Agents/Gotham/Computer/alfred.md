...
Trigger explicitly:
- `/morning-brief` (every weekday morning)
...
### Sub-agents to spawn
- **`task-analyzer`** — spawn on every `/morning-brief` and `/today`; reads `Tasks/active.md`, returns top 3 blockers and today's focus
- **`project-scanner`** — spawn on `/briefing`; scans `Projects/` for stale milestones and upcoming deadlines
