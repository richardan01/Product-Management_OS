# Run protocol — go-nogo meta-eval
...
2. **Runner (isolated context), one per fixture.** The `eval-runner` sub-agent executes `/go-nogo <fixture>` and captures the **verbatim** gate output (full gate scorecard, verdict, conditions/blockers) to `results/transcripts/YYYY-MM-DD_<fixture>_<model>.md`.
...
5. **Introspection loop** on every ❌: show the runner its output + the missed criterion, ask *why*, capture the harness bug (a confusing line in `go-nogo/SKILL.md` or the severity-taxonomy gate table), and propose the fix.
6. **Log** the run to `results/YYYY-MM-DD_<model>.md` and append a summary row to `Evals/run-log.md`.
7. **Clear CI** via `/eval-ci clear go-nogo`.

## Map each fixture to its applicable evals

| Fixture | Evals to grade |
|---|---|
| `clean-launch-packet.md` | 01, 02, 03 |
| `flawed-launch-packet.md` | 01, 02, 04 (the 🔴 gate must force NO-GO, not CONDITIONAL GO) |
