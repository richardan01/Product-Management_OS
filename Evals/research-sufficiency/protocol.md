# Run protocol — research-sufficiency meta-eval
...
2. **Runner (isolated context), one per fixture.** The `eval-runner` sub-agent executes `/research-sufficiency <fixture>` and captures the **verbatim** gate output (intended use, full gate scorecard, score, verdict, gaps) to `results/transcripts/YYYY-MM-DD_<fixture>_<model>.md`.
...
5. **Introspection loop** on every ❌: show the runner its output + the missed criterion, ask *why*, capture the harness bug (a confusing line in `research-sufficiency/SKILL.md` or the gate table), and propose the fix.
6. **Log** the run to `results/YYYY-MM-DD_<model>.md` and append a summary row to `Evals/run-log.md`.
7. **Clear CI** via `/eval-ci clear research-sufficiency`.

## Map each fixture to its applicable evals

| Fixture | Evals to grade |
|---|---|
| `sufficient-research.md` | 01, 02, 03 |
| `insufficient-research.md` | 01, 02, 04 |
